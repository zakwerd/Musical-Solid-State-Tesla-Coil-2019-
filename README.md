...
1. A separate circuit is made to transform the 120VAC wall output into 5V and 12V outputs to power the ICs.  
2.The interrupter outputs a square-wave signal with adjustable pulse-width and frequency. The signal is sent to the gate driver to turn it on and off. When the driver chips are enabled, the hex inverter in the feedback system will send a short pulse allowing the oscillation to begin.  
3. The gate driver amplifies the feedback signal’s current and sends it to the inverter.  
4. 120VAC feeds into the two capacitors. The gate driver’s signal will trigger the top IGBT (arbitrarily chosen for this example) and leave the bottom IGBT off. Top capacitor will be unable to discharge. With the top IGBT open, the bottom capacitor can discharge through both diodes, through the top IGBT, and across the primary coil. When this ends, the IGBTs switch being on and off, discharging the other capacitor through the coil as the other recharges.
